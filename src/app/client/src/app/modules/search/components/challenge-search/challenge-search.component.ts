...
export class ChallengeSearchComponent implements OnInit {
    ...
    this.searchService.contentSearch(requestParams).subscribe(
      (apiResponse: ServerResponse) => {
        if (apiResponse.result.count && apiResponse.result.content) {
          ...
          const constantData = this.config.appConfig.ChallengeSearch.constantData;
        const metaData = this.config.appConfig.ChallengeSearch.metaData;
        const dynamicFields = this.config.appConfig.ChallengeSearch.dynamicFields;
  ...
  */
  navigateToPage(page: number): undefined | void {
    ...
    this.route.navigate(['search/Challenge', this.pageNumber], {
      queryParams: this.queryParams
    });
  }
  ngOnInit() {
    this.filterType = this.config.appConfig.challenge.filterType;
    this.redirectUrl = this.config.appConfig.challenge.searchPageredirectUrl;
